...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("GSSM - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("GSSM - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await sequelize.query(
            ...
            [process],
            [total] AS [prod_total],
                    LAG([total]) OVER (PARTITION BY[mc_no],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 7 AND 18 THEN
                            'M'
                        ELSE
                            'N'
                        END ORDER BY[registered]) AS[prev_ok],
                    LEAD([total]) OVER (PARTITION BY[mc_no],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 7 AND 18 THEN
                            'M'
                        ELSE
                            'N'
                ...
                FROM
            [data_machine_gssm].[dbo].[DATA_PRODUCTION_GSSM]
                WHERE
            ...
            FROM
            [calculated]
                LEFT JOIN [data_machine_gssm].[dbo].[DATA_MASTER_GSSM] m ON calculated.mc_no = m.mc_no
            GROUP BY
            [operation_day],
                calculated.[mc_no]
            ORDER BY
        ...
        // console.log("GSSM - data prod...", data[0]);
...
`
        ...
        console.log("GSSM - prod insert error:", error);
        return {
            data: error.message,
            success: true,
