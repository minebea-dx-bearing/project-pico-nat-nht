...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("FIM - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("FIM - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await sequelize.query(
            ...
            [total] AS[prod_total],
                ...
                FROM
            [data_machine_fim].[dbo].[DATA_PRODUCTION_FIM]
                WHERE
            ...
            [operation_day],
                ...
                'FIM' AS[process],
                CONCAT('LINE ', (CAST(
                        RIGHT (calculated.[mc_no], 2) AS INT))) AS line_name,
            ...
            FROM
            [calculated]
                LEFT JOIN[data_machine_fim].[dbo].[DATA_MASTER_FIM] m ON calculated.mc_no = m.mc_no
            GROUP BY
            [operation_day],
                calculated.[mc_no]
            ORDER BY
        ...
        // console.log("FIM - data prod...", data[0]);
...
`
        ...
        console.log("FIM - prod insert error:", error);
        return {
            data: error.message,
            success: true,
