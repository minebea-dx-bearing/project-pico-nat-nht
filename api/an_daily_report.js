...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("AN - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("AN - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await sequelize.query(
            ...
            [process],
            [tt] AS[prod_total],
                    LAG([tt]) OVER (PARTITION BY[mc_no],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 7 AND 18 THEN
                            'M'
                        ELSE
                            'N'
                        END ORDER BY[registered]) AS[prev_ok],
                    LEAD([tt]) OVER (PARTITION BY[mc_no],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 7 AND 18 THEN
                            'M'
                        ELSE
                            'N'
                ...
                FROM
            [data_machine_an2].[dbo].[DATA_PRODUCTION_AN]
                WHERE
            ...
            FROM
            [calculated]
                LEFT JOIN[data_machine_an2].[dbo].[DATA_MASTER_AN] m ON calculated.mc_no = m.mc_no
            GROUP BY
            [operation_day],
                calculated.[mc_no]
            ORDER BY
        ...
        // console.log("AN - data prod...", data[0]);
...
`
        ...
        console.log("AN - prod insert error:", error);
        return {
            data: error.message,
            success: true,
