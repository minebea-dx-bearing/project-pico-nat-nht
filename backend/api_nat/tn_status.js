...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - TN - New Running data status cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    console.log("NAT - TN - Use date in NewStatusGetDailyStatusReport...", dateToday, dateTomorrow);
    try {
        let data = await dbNAT.query(`
            DECLARE @start_date DATETIME = '${dateToday} 05:00'; -- เปลี่ยนวันที่ด้วย
            DECLARE @end_date DATETIME = '${dateTomorrow} 05:00'; -- เปลี่ยนวันที่ด้วย
            ...
            DECLARE @shift1 VARCHAR(8) = '05:00:00';
            DECLARE @shift2 VARCHAR(8) = '17:00:00';
            DECLARE @split_shift DATETIME = DATEADD(HOUR, 17, CAST(CAST(@start_date AS DATE) AS DATETIME));
            DECLARE @process VARCHAR(8) = 'TN';
            ...
            SELECT 
                ...
                [occurred],
                CASE WHEN DATEPART(HOUR, [occurred]) < 5 THEN CONVERT(date, DATEADD(DAY, -1, [occurred])) ELSE CONVERT(date, [occurred]) END AS [work_date],
                [mc_status] COLLATE DATABASE_DEFAULT AS [mc_status], 
                [occurred] AS [occurred_start]
            INTO #TempStatus
            FROM [nat_mc_mcshop_tn].[dbo].[DATA_MCSTATUS_TN]
            ...
            SELECT
                [mc_no] COLLATE DATABASE_DEFAULT AS [mc_no], 
                [process] COLLATE DATABASE_DEFAULT AS [process],
                CASE WHEN DATEPART(HOUR, [registered]) < 5 THEN CONVERT(date, DATEADD(DAY, -1, [registered])) ELSE CONVERT(date, [registered]) END AS [work_date],
            ...
            FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [nat_mc_mcshop_tn].[dbo].[DATA_PRODUCTION_TN] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
            WHERE NOT EXISTS (
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                await sequelize.query(`
                    INSERT INTO [NAT_DX_TO_PICO].[dbo].[TN_DAILY_STATUS_REPORT] ([operation_day],[is_operation_day],[process],[line_name],[machine_name],[status_name],[daily_duration_s],[daily_count],[shift1_duration_s],[shift1_count],[shift2_duration_s],[shift2_count],[shift3_duration_s],[shift3_count],[registered_at])
                    SELECT
                    ...
                    WHERE
                        NOT EXISTS ( 
                            SELECT 1
                            FROM [NAT_DX_TO_PICO].[dbo].[TN_DAILY_STATUS_REPORT]
            ...
            console.log("NAT - TN - Insert status new Done!");
            return {
                data: data[0],
                success: true,
            ...
            console.log("NAT - TN - Can't new insert : Length = 0");
        }
    } catch (error) {
        console.log("NAT - TN - new status insert error:", error);
        return {
            data: error.message,
            success: true,
