...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        let data = await dbNAT.query(`
            ...
            INTO #TempStatus
            FROM [nat_mc_assy_avs].[dbo].[DATA_MCSTATUS_AVS]
            ...
            FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [nat_mc_assy_avs].[dbo].[DATA_PRODUCTION_AVS] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
            WHERE NOT EXISTS (
