...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        let data = await dbNAT.query(`
            ...
            INTO #TempStatus
            FROM [nat_mc_assy_fim].[dbo].[DATA_MCSTATUS_FIM]
            ...
            FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [nat_mc_assy_fim].[dbo].[DATA_PRODUCTION_FIM] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
            WHERE NOT EXISTS (
