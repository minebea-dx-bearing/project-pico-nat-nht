...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        ...
        DECLARE @process VARCHAR(8) = 'GSSM';
        ...
        INTO #TempStatus
        FROM [data_machine_gssm].[dbo].[DATA_MCSTATUS_GSSM]
        ...
        FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_gssm].[dbo].[DATA_PRODUCTION_GSSM] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
        WHERE NOT EXISTS (
