...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        let data = await sequelize.query(`
            ...
            INSERT INTO #TempMerge (mc_no, process, work_date, mc_status, occurred_start)
            SELECT a.mc_no, 'AOD', CAST(@start_date AS DATE), 'connection lost', @start_date
            FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_aod].[dbo].[DATA_MASTER_AOD]) a
            WHERE NOT EXISTS (
