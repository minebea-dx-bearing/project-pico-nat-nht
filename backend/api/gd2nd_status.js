...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        ...
        INSERT INTO #TempMerge (mc_no, process, work_date, mc_status, occurred_start)
        SELECT a.mc_no, 'GD', CAST(@start_date AS DATE), 'connection lost', @start_date
        FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_gd2].[dbo].[DATA_MASTER_GD]) a
        WHERE NOT EXISTS (
...
// เรียกใช้
// getDaily('2025-09-01'); 
// NewStatusGetDailyStatusReport('2026-06-29'); // For All M/C
