...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
  ...
  try {
    let data = await sequelize.query(`
        ...
        INTO #TempStatus
        FROM [data_machine_fim].[dbo].[DATA_MCSTATUS_FIM]
        ...
        FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_fim].[dbo].[DATA_PRODUCTION_FIM] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
        WHERE NOT EXISTS (
