...
cron.schedule(
  ...
  async () => {
    ...
    console.log(
      "NHT - ALU - New Running data status cron job for date:",
      dateToday,
      hours,
...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
  ...
  console.log(
    "NHT - ALU - Use date in NewStatusGetDailyStatusReport...",
    dateToday,
    dateTomorrow
  );
  try {
    let data = await sequelize.query(`
        ...
        FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_alu].[dbo].[DATA_PRODUCTION_ALU] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
        WHERE NOT EXISTS (
