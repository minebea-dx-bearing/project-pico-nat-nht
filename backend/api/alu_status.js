    ...
    try {
        let dataMD = await sequelize.query(`
            ...
            WITH [base_alarm] AS (
                ...
                WHERE [occurred] BETWEEN @start_date_p1 AND @end_date_p1 AND mc_no LIKE 'ALUMD%'
            ),
            [with_pairing] AS (
                SELECT *,
            ...
            [base_monitor_iot] AS (
                ...
                WHERE registered BETWEEN @start_date_p1 AND @end_date_p1 AND mc_no LIKE 'ALUMD%'
            ),
            [mark] AS (
                SELECT
                    [mc_no],
        ...
        // convert จาก code Jelly เพราะมันนานกว่า 10 min
        let dataMA = await sequelize.query(`
            ...
            SELECT 
                ...
                [occurred] AS [occurred_start]
            INTO #TempStatus
            FROM [data_machine_alu].[dbo].[DATA_MCSTATUS_ALU]
            WHERE [occurred] >= DATEADD(DAY, -1, @start_date) AND [occurred] <= @end_date
              AND mc_no LIKE 'ALUMA%';
            ...
            INSERT INTO #TempMerge (mc_no, process, work_date, mc_status, occurred_start)
            SELECT a.mc_no, 'ALU', CAST(@start_date AS DATE), 'connection lost', @start_date
            FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_alu].[dbo].[DATA_MASTER_ALU] WHERE mc_no LIKE 'ALUMA%') a
            WHERE NOT EXISTS (
