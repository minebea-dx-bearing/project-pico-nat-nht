...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        ...
        FROM (SELECT DISTINCT mc_no COLLATE DATABASE_DEFAULT AS mc_no FROM [data_machine_an2].[dbo].[DATA_PRODUCTION_AN] WHERE registered >= DATEADD(DAY, -1, @start_date)) a
        WHERE NOT EXISTS (
