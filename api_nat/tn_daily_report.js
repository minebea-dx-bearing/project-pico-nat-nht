        ...
        let data = await dbNAT.query(`
            ...
            SET @SQL = '
                WITH [raw_data] AS (
                    SELECT
                        [registered],
                        CASE WHEN DATEPART(HOUR, registered) < 5 THEN CONVERT(date, DATEADD(DAY, -1, registered))
                            ELSE CONVERT(date, registered)
                        END AS [operation_day],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 5 AND 16 THEN ''M'' ELSE ''N'' END AS[shift_mn],
                        [mc_no],
                        [process],
                    ...
                    FROM ' + @Database + '
                    WHERE [registered] BETWEEN ''${dateToday} 04:00'' AND ''${dateTomorrow} 05:00''
                ),
                [calc_shift] AS (
                    SELECT
