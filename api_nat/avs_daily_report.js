...
const getDailyReport = async (dateQuery) => {
    ...
    try {
        let data = await dbNAT.query(`
            ...
            SET @Columns = '[daily_ok] + [daily_ag1] + [daily_ag2]';
            SET @Database = '[nat_mc_assy_avs].[dbo].[DATA_PRODUCTION_AVS]';
        ...
        `);
        // console.log("NAT - AVS - data prod...", data[0]);
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NAT_DX_TO_PICO].[dbo].[AVS_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NAT_DX_TO_PICO].[dbo].[AVS_DAILY_REPORT]
                        WHERE
