...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - ANT - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NAT - ANT - prod...", dateToday, dateTomorrow);

    try {
        let data = await dbNAT.query(`
            DECLARE @ColumnsR NVARCHAR(MAX);
            DECLARE @ColumnsF NVARCHAR(MAX);
            DECLARE @Database NVARCHAR(MAX);
            DECLARE @LineName NVARCHAR(MAX);
            DECLARE @SQL NVARCHAR(MAX);

            SET @ColumnsR = '[ok_rear]+[ag_rear]+[ng_rear]+[mixball_rear]';
            SET @ColumnsF = '[ok_front]+[ag_front]+[ng_front]+[mixball_front]';
            SET @Database = '[nat_mc_assy_ant_new].[dbo].[DATA_PRODUCTION_ANT]';
            SET @LineName = 'CAST(RIGHT(s.[mc_no],2) AS INT)';

            -- อย่าลืมแก้เวลาตัดกะ

            SET @SQL = '
                WITH [raw_data_antr] AS (
                    SELECT
                        [registered],
                        ...
                        [mc_no],
                        [process],
                        ' + @ColumnsR + ' AS [prod_total],
                        CASE WHEN ' + @ColumnsR + ' - LAG(' + @ColumnsR + ') OVER (PARTITION BY mc_no ORDER BY registered) < 0 THEN ' + @ColumnsR + '
                            ELSE ' + @ColumnsR + ' - LAG(' + @ColumnsR + ') OVER (PARTITION BY mc_no ORDER BY registered)
                ...
                [raw_data_antf] AS (
                    SELECT
                        ...
                        [mc_no],
                        [process],
                        ' + @ColumnsF + ' AS [prod_total],
                        CASE WHEN ' + @ColumnsF + ' - LAG(' + @ColumnsF + ') OVER (PARTITION BY mc_no ORDER BY registered) < 0 THEN ' + @ColumnsF + '
                            ELSE ' + @ColumnsF + ' - LAG(' + @ColumnsF + ') OVER (PARTITION BY mc_no ORDER BY registered)
                        END AS [prod_total_diff]
                    FROM ' + @Database + '
                    WHERE [registered] BETWEEN ''${dateToday} 06:00'' AND ''${dateTomorrow} 07:00''
                ),
        ...
        `);
        // console.log("NAT - ANT - data prod...", data[0]);
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NAT_DX_TO_PICO].[dbo].[ANT_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NAT_DX_TO_PICO].[dbo].[ANT_DAILY_REPORT]
                        WHERE
        ...
        console.log("NAT - ANT - prod insert error:", error);
        return {
            data: error.message,
            success: true,
