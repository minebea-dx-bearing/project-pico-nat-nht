...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - GD2ND - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NAT - GD2ND - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await dbNAT.query(`
            ...
            SET @Columns = '[prod_total] + [ng_p] + [ng_n]';
            SET @Database = '[nat_mc_mcshop_2gd].[dbo].[DATA_PRODUCTION_2GD]';
            SET @LineName = 'CAST(LEFT(RIGHT(s.[mc_no],3),2) AS INT)';
            ...
            SET @SQL = '
                WITH [raw_data] AS (
                    SELECT
                        [registered],
                        CASE WHEN DATEPART(HOUR, registered) < 8 THEN CONVERT(date, DATEADD(DAY, -1, registered))
                            ELSE CONVERT(date, registered)
                        END AS [operation_day],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 8 AND 19 THEN ''M'' ELSE ''N'' END AS[shift_mn],
                        [mc_no],
                        [process],
                    ...
                    FROM ' + @Database + '
                    WHERE [registered] BETWEEN ''${dateToday} 07:00'' AND ''${dateTomorrow} 08:00''
                ),
                [calc_shift] AS (
                    SELECT
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NAT_DX_TO_PICO].[dbo].[GD2ND_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NAT_DX_TO_PICO].[dbo].[GD2ND_DAILY_REPORT]
                        WHERE
        ...
        console.log("NAT - GD2ND - prod insert error:", error);
        return {
            data: error.message,
            success: true,
