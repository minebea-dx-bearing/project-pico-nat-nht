...
const getDailyReport = async (dateQuery) => {
    ...
    try {
        let data = await dbNAT.query(`
            ...
            SET @Columns = '[prod_cnt]';
            SET @Database = '[nat_mc_assy_alu].[dbo].[DATA_PRODUCTION_ALU]';
        ...
        `);
        // console.log("NAT - ALU - data prod...", data[0]);
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NAT_DX_TO_PICO].[dbo].[ALU_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NAT_DX_TO_PICO].[dbo].[ALU_DAILY_REPORT]
                        WHERE
