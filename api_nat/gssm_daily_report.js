...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - GSSM - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NAT - GSSM - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await dbNAT.query(
            ...
            [mc_no],
            [process],
            ([grease_ok]+[ro1_ng]+[ro2_ng]+[grease_ng]+[shield_ok]+[shield_a_ng]+[shield_b_ng]+[snap_a_ng]+[snap_b_ng]) AS [prod_total],
                    LAG(([grease_ok]+[ro1_ng]+[ro2_ng]+[grease_ng]+[shield_ok]+[shield_a_ng]+[shield_b_ng]+[snap_a_ng]+[snap_b_ng])) OVER (PARTITION BY[mc_no],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 7 AND 18 THEN
                            'M'
                        ELSE
                            'N'
                        END ORDER BY[registered]) AS[prev_ok],
                    LEAD(([grease_ok]+[ro1_ng]+[ro2_ng]+[grease_ng]+[shield_ok]+[shield_a_ng]+[shield_b_ng]+[snap_a_ng]+[snap_b_ng])) OVER (PARTITION BY[mc_no],
                        CASE WHEN DATEPART (HOUR, registered) BETWEEN 7 AND 18 THEN
                            'M'
                        ELSE
                            'N'
                ...
                FROM
            [nat_mc_assy_gssm].[dbo].[DATA_PRODUCTION_GSSM]
                WHERE
            ...
            [operation_day],
                ...
                'GSSM' AS[process],
				CONCAT('LINE ', (CAST(RIGHT(calculated.[mc_no], 2) AS INT)))  AS line_name,
            ...
            FROM
            [calculated]
                LEFT JOIN [nat_mc_assy_gssm].[dbo].[DATA_MASTER_GSSM] m ON calculated.mc_no = m.mc_no
            GROUP BY
            [operation_day],
                calculated.[mc_no]
            ORDER BY
        ...
        // console.log("NAT - GSSM - data prod...", data[0]);
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(
                    `
                INSERT INTO  [NAT_DX_TO_PICO].[dbo].[GSSM_DAILY_REPORT] (
                ...
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM  [NAT_DX_TO_PICO].[dbo].[GSSM_DAILY_REPORT]
                    WHERE
...
`
        ...
        console.log("NAT - GSSM - prod insert error:", error);
        return {
            data: error.message,
            success: true,
