...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - AOD - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NAT - AOD - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await dbNAT.query(`
            ...
            SET @Columns = '[daily_ok] + [daily_ag]';
            SET @Database = '[nat_mc_assy_aod].[dbo].[DATA_PRODUCTION_AOD]';
        ...
        `);
        // console.log("NAT - AOD - data prod...", data[0]);
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NAT_DX_TO_PICO].[dbo].[AOD_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NAT_DX_TO_PICO].[dbo].[AOD_DAILY_REPORT]
                        WHERE
        ...
        console.log("NAT - AOD - prod insert error:", error);
        return {
            data: error.message,
            success: true,
